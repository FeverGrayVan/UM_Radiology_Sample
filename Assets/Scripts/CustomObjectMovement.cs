...
public class CustomObjectMovement : MonoBehaviour
{
   ...
   //private Slider _slider;
    ...
    public static float moveValue =0.5f;
    ...
    private void MoveSlider()
    {
       // _projectedPositionOnLine = NearestPointOnFiniteLine(_startPoint.position, _endPoint.position, _cursor.transform.position);
        ...
        moveValue = maxValue - (_projectedPositionOnLineDistanceFromEnd / _totalLineLength) * maxValue;
        //  _slider.value = (float)Math.Round(_slider.value, 0);
        Debug.Log("move value " + moveValue);
        // Debug.Log("slider value-------" + _slider.value);

        //if (thresholdIntensityPanel.activeInHierarchy)
        //{
